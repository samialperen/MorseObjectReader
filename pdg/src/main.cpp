...
int main(int argc, char** argv) {
    ...
    if (node.hasParam("/pdg/MorseObjectReader"))
        node.getParam("/pdg/MorseObjectReader", morseObject_);
    ...
    MorseObjectReader morseObjectRd(node, "/bluecup_pose"); //ortadaki morseObjectRd den emin değilim.. 
    ...
    while (node.ok()) {
        ...
        ////ALPEREN////////////////////////////////////////////////
	if (morseObject_)
            morseObjectRd.updateObjects(listener);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if (morseObject_) {
            for (std::map<std::string, MovableObject *>::iterator it = morseObjectRd.lastConfig_.begin();
                 it != morseObjectRd.lastConfig_.end(); ++it) {
                if (newPoseEnt_.getId() == it->first) {
                    updateEntity(newPoseEnt_, it->second);
                    //Reset newPoseEnt_
                    newPoseEnt_.setId("");
